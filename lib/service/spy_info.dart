...
Future<List<String>> fetchSpyPrice([bool isDay = true]) async {
  final response = await Client().post(
      ...
      body: {
        ...
        'commodity_id': 'TX',
        'queryDate': DateFormat('yyyy/MM/dd').format(spyDate(isDay)),
        'MarketCode': '0',
        'commodity_idt': 'TX'
      });
  String high = '';
...
DateTime spyDate(bool isDay, [DateTime? time]) {
  DateTime spyDate = time ?? DateTime.now().toUtc().add(const Duration(hours: 8));
